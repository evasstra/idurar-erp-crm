import { useCallback } from 'react'; // Move import to the top
...
const getLabel = (key) => {
  try {
    ...
    if (!result) {
      const list = {}; // Change let to const
      list[lowerCaseKey] = label;
      window.localStorage.setItem('lang', JSON.stringify(list));
    } else {
      const list = { ...JSON.parse(result) }; // Change let to const
      list[lowerCaseKey] = label;
...
const useLanguage = () => {
  // Wrap the returned function in useCallback to stabilize it
  const translate = useCallback((value) => {
    return getLabel(value);
  }, []); // Empty dependency array as getLabel doesn't depend on hook's props/state
