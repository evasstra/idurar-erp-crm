...
const updateManySetting = async (req, res) => {
  ...
  for (const setting of settings) {
    ...
    const updateOperation = {
      ...
      $setOnInsert: {
        settingCategory: settingCategory, // Set category on insert
        settingKey: settingKey,
        valueType: 'string', // Assuming string for now, might need refinement if types vary
        isPrivate: false, // Default from schema
        isCoreSetting: false, // Default from schema
        enabled: true, // Default from schema
        removed: false, // Default from schema
      },
    };
